...
function pantheon_decoupled_umami_demo_form_install_configure_form_alter(&$form, FormStateInterface $form_state) {
  ...
  $form['#submit'][] = 'pantheon_decoupled_umami_demo_form_install_configure_submit';
}

/**
 * Submission handler to sync the contact.form.feedback recipient.
 */
function pantheon_decoupled_umami_demo_form_install_configure_submit($form, FormStateInterface $form_state) {
  $site_mail = $form_state->getValue('site_mail');
  ContactForm::load('feedback')->setRecipients([$site_mail])->trustData()->save();

  $password = $form_state->getValue('account')['pass'];
  pantheon_decoupled_umami_demo_set_users_passwords($password);
}

/**
 * Sets the password of admin to be the password for all users.
 */
function pantheon_decoupled_umami_demo_set_users_passwords($admin_password) {
 ...
 */
function pantheon_decoupled_umami_demo_toolbar() {
